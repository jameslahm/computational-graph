...
int main()
{
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        if (TempStr == "SIN" || TempStr == "LOG" || TempStr == "EXP" || TempStr == "TANH" ||
			...
			Sample_Graph.BuildCalcNode<BindNode<double>>(NodeName, 2, OperandsList);
		} else if(TempStr == "ASSIGN"){
            for (int j = 1; j <= 2; ++j) {
                cin >> TempStr;
                OperandsList.push_back(TempStr);
			}
			Sample_Graph.BuildCalcNode<AssignCNode<double>>(NodeName,2,OperandsList);
		} else if(TempStr=="GRAD"){
            cin>>TempStr;
            OperandsList.push_back(TempStr);
            Sample_Graph.BuildCalcNode<GradNode<double>>(NodeName,1,OperandsList);
        }
            ...
            if (OPRT=="AT")
                Sample_Graph.BuildCalcNode<GradCNode<double>>(NodeName,2,OperandsList);
            if (OPRT == "+")
