...
template <>
double EQCNode<double>::grad(Node<double> *node)
{ //EQCNode不可求导，直接但会0，并输出提示信息
    cout << "EQCNode "
         << "NO GRAD" << endl;
    double res = double(0);
    return res;
...
template <>
Tensor<double> EQCNode<Tensor<double>>::grad(Node<Tensor<double>> *node)
{ //特化Tensor类型
    cout << "EQCNode "
         << "NO GRAD" << endl;
    Tensor<double> res(0);
...
template <>
Tensor<double> EQCNode<Tensor<double>>::Calc() //特化Tensor类型
{
